...
export function overrideComponents(
  ...
  for (const override of overrides) {
    ...
    if (starlightConfig.components?.[name]) {
      const fallback = `./plugins/theme-rapide/${typeof override === 'string' ? 'overrides' : 'components'}/${typeof override === 'string' ? override : override.fallback}.astro`

      logger.warn(`A \`<${name}>\` component override is already defined in your Starlight configuration.`)
      logger.warn(
        `To use \`theme-rapide\`, either remove this override or manually render the content from \`${fallback}\`.`,
      )
      continue
    }
    components[name] = `./plugins/theme-rapide/overrides/${name}.astro`
  }
